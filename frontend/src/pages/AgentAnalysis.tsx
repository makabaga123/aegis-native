...
import { analyzeSingle, analyzeSingleFiles } from '../lib/api';
...
export function AgentAnalysis() {
  ...
  async function run() {
    ...
    try {
      if (mode === 'json') {
        const body: Record<string, unknown> = { persist: false };
        ...
        if (runtimeJson.trim()) {
          try { body.runtime_events = JSON.parse(runtimeJson); } catch { body.runtime_events = runtimeJson; }
        }
        setResult(await analyzeSingle(body));
      } else {
        ...
        setResult(await analyzeSingleFiles(fd));
      }
    ...
    <main className="pt-28 pb-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-3xl md:text-5xl font-extrabold tracking-tighter mb-8 text-white">
          AI <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">AGENT</span>
        </h2>
        ...
        <div className="space-y-6 mb-8">
          ...
          <div>
            ...
            <textarea
              rows={4} value={runtimeJson} onChange={(e) => setRuntimeJson(e.target.value)}
              placeholder='[{"event_type":"execve",...}]'
        ...
        {loading && <LoadingSpinner label="Agent analyzing..." />}
